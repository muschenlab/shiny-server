...
# saveRDS(es, "mile_data/MILE_leukemias_ES_lin.rds")

################################# Setup #######################################

# load data
es <- readRDS("mile_data/MILE_leukemias_ES_lin.rds")
...
server <- function(input, output) {
  ...
  gen_boxplot <- reactive({
    ...
    plot <- ggplot(plotdat, aes(x = group, y = expr)) +
      ...
      geom_segment(data = cat_lines, lwd = 2, alpha = .5,
  ...
  gen_barplot <- reactive({
    ...
    plot <- ggplot(plotdat, aes(x = group, y = average)) +
      geom_bar(aes(fill = lineage), alpha = 0.75, width = 0.8,
  ...
  output$main_plot_ui <- renderUI ({
    ...
    plotOutput("main_plot", width = plotdat$w*80, height = plotdat$h*80)
  })
