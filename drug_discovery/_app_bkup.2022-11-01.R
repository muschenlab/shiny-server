...
server <- function(input, output, session) {
    ...
    get_drug_dep_summary <- reactive({
        ...
        if (is_empty(drug_summary) | is_empty(reactvals$ct1) | is_empty(reactvals$ct2)) return(NULL)
    ...
    output$cl_drugdat <- DT::renderDataTable({
        ...
        if(is_empty(selcpd_metrics) | is_empty(reactvals$ct1)) return(NULL)
    ...
    get_crispr_av <- reactive({
        ...
        idx1 <- which(crispr$disease==ct1)
        idx2 <- which(crispr$disease==ct2)
    ...
    get_crispr_dat <- reactive({
        ...
        idx2 <- which(crispr$disease==ct2)
        av <- get_crispr_av()
        crispr <- data$crispr_es[, c(idx1, idx2)] 
        ...
        crisprdat <- exprs(crisprsub) %>%
            ...
            mutate(disease = pData(crisprsub)[match(.$cl_id, crisprsub$DepMap_ID),]$disease) %>%
            filter(!is.na(value)) 
        print("gather end")
        return(crisprdat)
    })
